 ...
 *
 * - aiArticleWriter - A function that generates a full article.
 */

import {ai} from '@/ai/genkit';
import {z} from 'genkit';
import { getSerpResults, SerpResult } from '@/services/serp-service';
import { AiArticleWriterInputSchema, AiArticleWriterOutputSchema, type AiArticleWriterInput, type AiArticleWriterOutput } from '@/ai/schema/ai-article-writer';
...
const writerPrompt = ai.definePrompt({
    name: 'aiArticleWriterPrompt',
    input: { schema: z.object({
        ...AiArticleWriterInputSchema.shape,
...
export async function getSerpAnalysis(input: AiArticleWriterInput): Promise<SerpResult[]> {
    return getSerpResults(input.primaryKeyword);
}


export async function generateArticleFromSerp(input: AiArticleWriterInput, serpResults: SerpResult[]): Promise<AiArticleWriterOutput> {
