...
import type { OneClickWriterSerpInput, OneClickWriterSerpOutput } from "@/ai/flows/one-click-writer-serp";
...
export async function getSerpAnalysisAction(
  formData: FormData
    ...
    try {
        ...
        if (serpResults.length === 0 && (!keywordData.search_volume && !keywordData.cpc) && relatedQuestions.length === 0) {
